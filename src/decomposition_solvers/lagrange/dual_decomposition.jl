...
mutable struct LagrangeSolverData
    ...
    upper_bound_heurstic::Union{Nothing,Function}  #e.g. fix binaries, fix master problem solution
    multiplier_update_method::Function #subgradient, cutting plane, etc...
    ...
    function LagrangeSolverData()
        ...
        data.upper_bound_heurstic = nothing
        data.multiplier_update_method = subgradient!
        return data
    end
end

#IDEA: Create a LagrangeModel using ModelGraph information
mutable struct LagrangeModel <: AbstractLagrangeModel
    ...
    link_eq_matrix::Union{Nothing,AbstractMatrix}
    link_ineq_matrix::Union{Nothing,AbstractMatrix}

    b_eq::Vector{Float64}
    b_ineq::Vector{Float64}

    #Vectors of JuMP variables
...
LagrangeModel() = LagrangeModel(LagrangeSolverData(),LagrangeSolution(),Vector{JuMP.Model}(),
                                ...
                                Vector{JuMP.VariableRef}(),Vector{JuMP.VariableRef}(),-Inf)
...
function LagrangeModel(graph::ModelGraph)
    ...
    #This actually updates the submodels with multiplier lists
...
function solve(lmodel::LagrangeModel)
    start_time = time()
    ...
    for iter in 1:max_iterations
        ...
        x_eq = value.(lmodel.link_eq_variables)
        x_ineq = value.(lmodel.link_ineq_variables)

        # Update residuals for multplier calculation
        residuals_equality = A_eq*x_eq - lmodel.b_eq

        #println(residuals_equality)
        residuals_inequality = A_ineq*x_ineq - lmodel.b_ineq

        # Check convergence
        if norm(residuals_equality) < epsilon && all(residuals_inequality .<= 0)
            return :Optimal
        end
        ...
        # end
        # Update multipliers
        eq_multipliers_delta, ineq_multipliers_delta = subgradient!(lmodel,residuals_equality,residuals_inequality)
